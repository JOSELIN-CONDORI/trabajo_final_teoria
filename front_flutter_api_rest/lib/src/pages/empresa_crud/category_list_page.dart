...
import 'package:front_flutter_api_rest/src/pages/empresa_crud/category_create_page.dart';
import 'package:front_flutter_api_rest/src/pages/empresa_crud/category_edit_page.dart';
import 'package:front_flutter_api_rest/src/pages/empresa_crud/category_show_page.dart';
import 'package:front_flutter_api_rest/src/providers/theme.dart';
import 'package:intl/intl.dart';
import 'package:provider/provider.dart';

class EmpresalistPage extends StatefulWidget {
  @override
  _EmpresalistPageState createState() => _EmpresalistPageState();
}

class _EmpresalistPageState extends State<EmpresalistPage> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();

  List<EmpresaModel> item = [];
  ...
  Future<void> _getData({String? nombre}) async {
    try {
      final categoriesData =
          await empresaController.getDataEmpresas(nombre: nombre);
      setState(() {
        item = categoriesData
            .map<EmpresaModel>((json) => EmpresaModel.fromJson(json))
            .toList();
      });
    } catch (error) {
  ...
  Future<void> _removeEmpresa(int id, String fotoURL) async {
    final response = await empresaController.removeEmpresa(id, fotoURL);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return ConfirmDeleteDialog(
          id: id,
          fotoURL: fotoURL,
          onConfirmDelete: _removeEmpresa,
        );
      },
    );
  }
        ...
        child: Column(
          children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => EmpresaCreatePage(),
                        ),
                      );
                    },
                    child: ClipPath(
            ...
            item.isEmpty
                ? ErrorData()
                : Column(
                    children: item.map<Widget>((empresa) {
                      return Dismissible(
                        key: Key(empresa.id.toString()), // Clave única
                        ...
                        onDismissed: (direction) {
                          _removeEmpresa(empresa.id!, empresa.foto!);
                        },
                        child: Column(
                          children: [
                            Container(
